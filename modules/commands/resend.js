  ...
  if (senderID == global.data.botID) return;
    ...
    if(event.type == "message_unsend") {
     ...
     let name = await Users.getNameUser(senderID);
      if(getMsg.attachment[0] == undefined) return api.sendMessage(`Kích hoạt Thanos Time Stone: ${name} đã cố xóa 1 tin nhắn. \nNội dung: ${getMsg.msgBody}`,threadID)
      else {
            let num = 0
            let msg = {
              body:`Kích hoạt Thanos Time Stone: ${name} vừa gỡ ${getMsg.attachment.length} tệp đính kèm.${(getMsg.msgBody != "") ? `\n\nNội dung: ${getMsg.msgBody}` : ""}`,
              attachment:[],
